...
- **Live Cart / takeover (ephemeral):** `CustomerScreenLiveState` in the cache store only (cart 15 min, takeover 60 s, per-instance sequence guard, takeover fence, sign-out cleanup via `ClearCustomerScreenCartsOnLogout`). `CustomerScreenCart` derives lines from ids. `useCustomerScreenCart` (POS) debounces 300 ms and serializes writes; the takeover is announced after Pay Now / Pay Later success, never inside them.
- **Queue position:** `KitchenBoard::queuePosition()` (same scope and order as the Customer Display board, same order type only).
