...
{
    public function execute(Order $requested, string $reason = 'stale_30_minutes'): bool
    {
        return DB::transaction(function () use ($requested, $reason): bool {
            ...
            $order->update(['commercial_status' => CommercialStatus::ArchivedUnclaimed,
                'archive_reason' => $reason, 'archived_at' => now(), 'version' => $order->version + 1]);
