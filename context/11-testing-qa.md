...
- New Pest files (45 tests): `CustomerScreenPairingTest` (one-time/expired/reused codes, one station ↔ one screen, cashier change keeps pairing, mutually exclusive modes / both off = Ads, Branch isolation, forged station, public screen cannot call staff routes, channel authorization, screen reset, kiosk page without shared props/manifest), `CustomerScreenLiveCartTest` (server-derived safe projection, foreign Products dropped, stale/out-of-order sends, empty-cart and sign-out cleanup, cross-station isolation, Dine In 3 s / Take Out 5 s takeovers with same-type queue position, pickup QR, mode preserved, takeover fence, recent-order only, canonical queue order), `CustomerScreenMenuMediaTest` (browse-only Menu parity and privacy, RBAC/Branch scope, image/MP4/HEVC/length/size/disguised-file validation, sequence/active/playlist privacy), `OrderPickupTest` (one strong token per committed Take Out incl. Pay Later and edits, none for Dine In or failed commits, allowlisted public status, raw id/hash/guess/expired, lifecycle incl. void and type change, one compact fan-out event, rate limit, no mutation, explicit idempotent per-order opt-in, channel authorization), `PickupBuzzTest` (Buzz only for Ready Take Out with a subscription, one push with the correct payload/URL, 5-s cooldown and max 5, idempotent key, POS/Branch authorization, rejected endpoint cleanup, bounded transient retry, Kitchen Ready never buzzes, Ready modal props).
- PostgreSQL harness `tests/verify-customer-experience-postgres.php` (opt-in, `DB_URL=null`, isolated `cx_*` schema, dropped): A migration rollback/reapply + indexes + mode CHECK; B 4 concurrent Buzzes → 1 accepted / 3 cooldown; C 3 concurrent replays of one key → 1 send; D racing same-station pairing → retry releases the previous screen. **Passed.**
- Frontend: new `tests/customer-screen.test.ts` and `tests/pickup.test.ts`; updated `kitchen-ui` (Ready events) and `pwa-contracts` (public-surface lists). Suite 316/316.
- Also: lint 0/0, TypeScript (app + service worker), PHPStan 0, Pint, production build, SQLite fresh/rollback/reapply on a disposable file, `git diff --check`. Normal development DB: forward `migrate` only.
- Focused regression set (the 45 new tests plus Customer Display, Kitchen, Pay Now/Later/settlement, committed edits, void, Customer QR, catalog, product images, Web Push, PWA shell, Settings, Access Control/Custom Roles/Branch-scoped management, realtime, performance, audit, auth): **830 passed / 6,472 assertions, 0 failed** (86 s). The complete Laravel suite was not run for this implementation pass (Phase 20 runs the full hardening suite).
- **Manual QA: NOT YET PASSED — awaiting the user** (checklist in `13-progress-tracker.md`).
