...
`php artisan operations:seed-qa` (`LocalOperationsQaSeeder`, local/testing only, never in DatabaseSeeder) ensures the Drinks Plan, Lemon drink recipes per Size, "Drink Add-ons" effects, "Instructions" and MAIN/QAVE Ingredient stock through the production actions. Reuse an Ingredient only when its base unit matches (locked mismatches keep the owner's record and use the QA fallback name, e.g. "Yakult Bottle", "Calamansi Juice"). Stock is brought to the QA starting balance with audited count corrections, never repeated opening balances or direct balance edits.
