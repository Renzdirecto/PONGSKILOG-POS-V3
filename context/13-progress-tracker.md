...
**Implementation delivered on `feature/transaction-history`; Phase 12 is not marked complete.** The cashier-only, branch-scoped History workspace, versioned committed-order editing, net inventory deltas, append-only reconciliation, balance settlement, grouped Payment attempts, private Cashless invoice proofs, canonical edit/proof audits, and compact POS/Kitchen invalidations are implemented. Void remains disabled for Phase 13. User manual device/visual QA and the separate final implementation audit remain pending.
