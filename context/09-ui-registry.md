...
- **Pamalengke Plans** (`operations.plans`): what a plan holds, plan cards (products with recipes, ingredients/shared, low stock/to buy, suggested market cost), Add/Edit plan dialog (name, description, icon, existing products with "in X plan" hints; moving warns "future sales only"; Archive), *Today across plans* table/cards with business totals, products-outside-plans note, and the one-stock-record explanation for a shared ingredient.
- **Overview**: seven KPI tiles (sales, est. COGS with uncosted amount, est. gross profit, est. market cost, below target, auto recommended, missing recipes), Needs attention, Upcoming pamamalengke (+ Summary), Today's consumption, Top-selling products, Recent ingredient movements.
...
- **Recipes**: Product → Recipe → Ingredient explainer, plan products (list ≥820px, chips below), product header with real prices, one recipe per size, rows with exact quantity/unit/estimated cost, add/remove, estimate boxes (cost, selling price, margin — margin needs every cost), unsaved-changes bar; Missing recipe state (Copy from size, No recipe needed, Start recipe); No recipe needed / Product-stock state.
