...
Branch table selection is optional for both Dine In and Take Out. If selected, the table must be active and belong to the current branch. Both Order Information / Pay Later and Pay Now show the active branch table choices for either order type. Changing order type preserves the selected table; No table / deselection is valid.
